...
class TempCNN(torch.nn.Module):
    def __init__(self, input_dim, n_type, n_date, n_dur, sequencelength, kernel_size=3, hidden_dims=64, dropout=0.4):
        super(TempCNN, self).__init__()
        self.modelname = f"TempCNN_input-dim={input_dim}_n_type={n_type}_n_date={n_date}_n_dur={n_dur}_sequencelenght={sequencelength}_" \
        ...
        self.dur_ = nn.Sequential(nn.Linear(4 * hidden_dims, n_dur), nn.LogSoftmax(dim=-1))
